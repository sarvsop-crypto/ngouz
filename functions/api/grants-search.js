...
async function handle({ request, env }, H) {
  ...
  const synthPrompt = [
    ...
    `Respond in: ${langName}.`,
    '',
    'Below are web search results from vetted donor / government sources. Include ONLY items that are a specific, currently-OPEN call for applications or active funding programme accepting submissions today.',
    '',
    'SKIP (do not include) — be strict, when in doubt drop:',
    '- News articles, press releases, blog posts ("X programme was launched", "winners announced", "results of the 2024 round").',
    '- Country profile pages, partnership overviews, "Where we work" / "What we do" pages, organisational descriptions.',
    '- Pages describing a PAST round with no current re-opening: language like "applications closed", "deadline was [past date]", "Round 2024", "Call 2023". A page that only retrospectively mentions a past programme is NOT a current grant.',
    `- Year-stamped calls older than ${nowYear} (e.g. "Programme 2024", "Call 2023") unless the page also clearly announces a current ${nowYear} re-opening.`,
    '- Vague "we support / we fund" statements without a named call, eligibility, or how to apply.',
    '',
